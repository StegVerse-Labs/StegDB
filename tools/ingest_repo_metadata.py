...
and aggregates them into a single JSONL index inside StegDB.

Usage (from StegDB repo root):

    python tools/ingest_repo_metadata.py
    python tools/ingest_repo_metadata.py --search-root _workspace

What it does:
- Searches for meta/files.jsonl under the given search root
- Reads each JSONL line (one per file in that repo)
- Adds derived fields (aggregated_at, discovered_from, repo)
- Writes a unified JSONL file:
...
DEFAULT_SEARCH_ROOT = STEGBDB_ROOT.parent
...
def find_metadata_files(search_root: Path) -> List[Path]:
    """
    Find all meta/files.jsonl under the given search root.
...
def ingest_metadata_file(path: Path) -> List[Dict[str, Any]]:
    """
    Read a meta/files.jsonl file and return parsed records.
    ...
    with path.open("r", encoding="utf-8") as f:
        for line_no, line in enumerate(f, start=1):
            ...
            try:
                ...
                print(f"⚠️ Skipping invalid JSON line in {path}:{line_no}: {exc}", file=sys.stderr)
                continue

            if not isinstance(obj, dict):
                print(f"⚠️ Skipping non-object JSON line in {path}:{line_no}", file=sys.stderr)
                continue
...
def aggregate_all_metadata(search_root: Path) -> int:
    """
    Aggregate all discovered meta/files.jsonl records into meta/aggregated_files.jsonl.
    ...
    if not discovered_files:
        print(f"⚠️ No meta/files.jsonl files found under {search_root}")
        return 0

    print(f"🔍 Found {len(discovered_files)} metadata file(s):")
...
def main() -> None:
    print("\n📦 StegDB Metadata Ingest\n")

    args = parse_args()
    search_root = Path(args.search_root).resolve()

    print(f"🔎 Searching for repo metadata under: {search_root}")

    count = aggregate_all_metadata(search_root)

    if count == 0:
        print("⚠️ No records aggregated. Check that validators have been run in repos.")
    else:
        rel = AGGREGATE_META_FILE.relative_to(STEGBDB_ROOT)
        print(f"\n✅ Aggregated {count} record(s) into {rel}")
