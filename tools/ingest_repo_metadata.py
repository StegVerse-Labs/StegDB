#!/usr/bin/env python3
"""
Ingest per-repo files.jsonl into a single aggregated_files.jsonl.

Input per repo (from generate_repo_metadata.py):

    repos/<RepoName>/files.jsonl

Output:

    meta/aggregated_files.jsonl

Each output line is the original record plus an explicit "repo" field.
"""
...
def ingest_all() -> None:
    ...
    with AGGREGATED.open("w", encoding="utf-8") as out:
        if not REPOS_DIR.exists():
            print("⚠ repos/ directory missing; nothing to ingest.")
            return
        ...
        for repo_dir in sorted(REPOS_DIR.iterdir()):
            ...
            files_path = repo_dir / "files.jsonl"
            if not files_path.exists():
                print(f"⚠ No files.jsonl for repo {repo_name}, skipping.")
                continue

            print(f"📥 Ingesting metadata for {repo_name} from {files_path}")
            with files_path.open("r", encoding="utf-8") as f:
                for line in f:
                    ...
                    if not line:
                        continue
                    rec = json.loads(line)
                    # ensure repo field present
                    rec.setdefault("repo", repo_name)
                    out.write(json.dumps(rec) + "\n")
                    count += 1

    print(f"✅ Wrote aggregated metadata to {AGGREGATED} ({count} records).")
    ...
    with path.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            try:
                ...
                print(f"⚠️ Skipping invalid JSON line in {path}: {exc}", file=sys.stderr)
                continue
            records.append(obj)
    return records
